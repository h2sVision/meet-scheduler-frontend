...
const Event = (props) => {
  ...
  const openSlots = (date) => {
    if (date) {
      for (
        let i = 0;
        i < document.getElementById("slots").children.length;
        i++
      ) {
  ...
  const sortSlots = async (minID, maxID) => {
    ...
    for (
      let dateId = new Date(minID);
      dateId <= new Date(maxID);
      dateId = new Date(dateId.setDate(dateId.getDate() + 1))
    ) {
  ...
  const countSlots = () => {
    ...
    weekContainers.forEach((weekContainer) => {
      ...
      const dayElements = weekContainer.querySelectorAll(
      ...
      dayElements.forEach((dayElement) => {
        ...
        slotElements.forEach((slotElement) => {
          ...
          const moderators = JSON.parse(
            slotElement.getAttribute("data-moderators")
          );
    ...
    // Opening working code for fetching and displaying slots
    ...
    //         `/user/${window.location.href.split("/")[4]}/userEventInfo`
    //     );
    ...
    //     for (
    //         let i = new Date(response?.data?.result?.start);
    //         i <= new Date(response?.data?.result?.end);
    ...
    //             for (
    //             let k = 0;
    //             k < moderators[i].availability[j].hours.length;
    //             k++
    //             ) {
    //             const startDate = new Date(
    //                 moderators[i].availability[j].start
    //             ).toDateString();
    //             const startTime = new Date(
    //                 moderators[i].availability[j].hours[k].start
    //             ).toTimeString();
    //             const compoundStart = new Date(startDate + " " + startTime);

    //             const endDate = new Date(
    //                 moderators[i].availability[j].end
    //             ).toDateString();
    //             const endTime = new Date(
    //                 moderators[i].availability[j].hours[k].end
    //             ).toTimeString();
    ...
    //                     if (
    //                     new Date(l) <
    //                         new Date(moderators[i].conferences[c].end) &&
    //                     tempL > new Date(moderators[i].conferences[c].start)
    //                     ) {
    //                     confOverlap = true;
    //                     }
    //                 }
    ...
    //                 } else {
    //                 l = new Date(
    //                     new Date(l.setDate(l.getDate() + 1)).toDateString() +
    //                     " " +
    //                     startTime
    //                 );
    //                 }
    ...
    //                 if (
    //                 new Date(moderators[i].customHours[j].date).toDateString() ===
    //                 initialslots[k].toDateString()
    //                 ) {
    ...
    //             for (
    //                 let k = 0;
    //                 k < moderators[i].customHours[j].hours.length;
    //                 k++
    //             ) {
    //                 const startDate = new Date(
    //                 moderators[i].customHours[j].date
    //                 ).toDateString();
    //                 const startTime = new Date(
    //                 moderators[i].customHours[j].hours[k].start
    //                 ).toTimeString();
    //                 const compoundStart = new Date(startDate + " " + startTime);
    ...
    //                 ).toDateString();
    //                 const endTime = new Date(
    //                 moderators[i].customHours[j].hours[k].end
    //                 ).toTimeString();
    //                 const compoundEnd = new Date(endDate + " " + endTime);

    //                 for (
    //                 let l = compoundStart;
    //                 new Date(l.getTime() + duration * 60000) <= compoundEnd;
    //                 l = new Date(
    //                     compoundStart.setMinutes(
    //                     compoundStart.getMinutes() + duration
    //                     )
    //                 )
    //                 ) {
    //                 let confOverlap = false;
    ...
    //             if (
    //                 !moderators[i].conferences.includes(
    //                 initialslots[slot].toISOString()
    //                 )
    //             ) {
    ...
    //                 if (ss?.children[0]) {
    //                 let mods = ss.children[0].dataset.moderators
    //                     .slice(1, -1)
    //                     .split(",");
    //                 if (mods.length > 1) {
    ...
    //                 newSlot.innerHTML = `<div data-moderators="[${
    //                     moderators[i].email
    //                 }]">${new Date(
    //                     initialslots[slot].setMinutes(
    //                     initialslots[slot].getMinutes() - duration
    //                     )
    //                 ).toLocaleString("en-US", {
    //                     hour: "numeric",
    //                     minute: "numeric",
    //                     hour12: true,
    //                 })}</div><div hidden>${initialslots[
    //                     slot
    //                 ].toISOString()}</div>`;
    //                 newSlot.children[0].classList.add("slotTime");
    //                 newSlot.addEventListener("click", async () => {
    //                     document.getElementById("displaySlot").innerHTML =
    //                     new Date(newSlot.children[1].innerHTML).toDateString() +
    //                     ", " +
    //                     new Date(
    //                         newSlot.children[1].innerHTML
    //                     ).toLocaleTimeString([], {
    //                         hour: "2-digit",
    //                         minute: "2-digit",
    //                         hour12: true,
    //                     }) +
    //                     new Date(newSlot.children[1].innerHTML)
    //                         .toString()
    //                         .slice(34);
    //                     setConference({
    //                     start: newSlot.children[1]?.innerHTML,
    //                     end: new Date(
    //                         new Date(newSlot.children[1]?.innerHTML).setMinutes(
    //                         new Date(
    //                             newSlot.children[1]?.innerHTML
    //                         ).getMinutes() + duration
    //                         )
    ...
    //                     for (
    //                     let tS = 0;
    //                     tS < document.getElementsByClassName("slotTime").length;
    //                     tS++
    //                     ) {
    //                     document
    //                         .getElementsByClassName("slotTime")
    //                         [tS].classList.remove("active");
    //                     }
    //                     newSlot.children[0].classList.add("active");
    //                 });

    //                 let dateDiv = document.getElementById(
    //                     initialslots[slot].toDateString()
    //                 );
    ...
    // New fetching code
const printData = async () => {
    console.log('🚀 Starting printData function');
    let availableSlotsCount = 0;
    ...
    try {
        ...
        const response2 = await axiosPrivate.get(`/user/${userId}/userEventInfo`);
        
        console.log('✅ API responses received');
        console.log('📊 User data:', response?.data?.result);
        console.log('📅 Event data:', response2?.data?.result);
        ...
        console.log('⏱️  Event duration:', duration, 'minutes');
        console.log('👥 Moderators count:', moderators?.length || 0);

        // Create date containers for the event period
        console.log('📅 Creating date containers...');
        const startDate = new Date(response?.data?.result?.start);
        const endDate = new Date(response?.data?.result?.end);
        
        for (let currentDate = new Date(startDate); currentDate <= endDate; currentDate.setDate(currentDate.getDate() + 1)) {
            let newDateDiv = document.createElement("div");
            newDateDiv.style.display = "none";
            newDateDiv.id = currentDate.toDateString();
            newDateDiv.classList.add("dateSlotsContainer");
            newDateDiv.innerHTML = `<div class='flex justify-center items-center text-center'>No Slots Available on this Day.</div>`;
            document.getElementById("slots")?.append(newDateDiv);
        }
        ...
        if (moderators && moderators.length > 0) {
            for (let i = 0; i < moderators.length; i++) {
                ...
                for (let j = 0; j < moderators[i].availability.length; j++) {
                    for (let k = 0; k < moderators[i].availability[j].hours.length; k++) {
                        try {
                            ...
                            let currentSlot = new Date(compoundStart);
                            
                            while (currentSlot < compoundEnd) {
                                let slotEnd = new Date(currentSlot.getTime() + duration * 60000);
                                if (slotEnd <= compoundEnd) {
                                    let confOverlap = false;
                                    
                                    for (let c = 0; c < moderators[i].conferences.length; c++) {
                                        let slotStart = new Date(currentSlot);
                                        let slotEndForConflict = new Date(currentSlot.getTime() + duration * 60000);
                                        let confStart = new Date(moderators[i].conferences[c].start);
                                        let confEnd = new Date(moderators[i].conferences[c].end);
                                        
                                        if (slotStart < confEnd && slotEndForConflict > confStart) {
                                            confOverlap = true;
                                            break;
                                        }
                                    }
                                    ...
                                    if (!confOverlap) {
                                        initialslots.push(new Date(currentSlot));
                                    }
                ...
                const processCustomHours = async () => {
                    try {
                        const customDates = new Set();
                        for (let j = 0; j < moderators[i].customHours.length; j++) {
                            const customDate = new Date(moderators[i].customHours[j].date).toDateString();
                            customDates.add(customDate);
                        }

                        initialslots = initialslots.filter(slot => !customDates.has(new Date(slot).toDateString()));

                        for (let j = 0; j < moderators[i].customHours.length; j++) {
                            const customDay = moderators[i].customHours[j];
                            for (let k = 0; k < customDay.hours.length; k++) {
                                try {
                                    const startDate = new Date(customDay.date).toDateString();
                                    const startTime = new Date(customDay.hours[k].start).toTimeString();
                                    const endTime = new Date(customDay.hours[k].end).toTimeString();
                                    
                                    const compoundStart = new Date(startDate + " " + startTime);
                                    const compoundEnd = new Date(startDate + " " + endTime);
                                    
                                    let currentSlot = new Date(compoundStart);
                                    
                                    while (currentSlot.getTime() + duration * 60000 <= compoundEnd.getTime()) {
                                        let confOverlap = false;
                                        ...
                                        for (let c = 0; c < moderators[i].conferences.length; c++) {
                                            ...
                                            if (slotStart < confEnd && slotEnd > confStart) {
                                                confOverlap = true;
                                                break;
                                            }
                                        }
                                    ...
                                    console.error('❌ Error processing custom hours:', error);
                                }
                            }
                        }
                    } catch (error) {
                        console.error('❌ Error in custom hours processing:', error);
                    }
                };

                const displaySlots = () => {
                    initialslots.sort((a, b) => new Date(a) - new Date(b));

                    const now = new Date();
                    const todayDateString = now.toDateString();

                    for (let slot = 0; slot < initialslots.length; slot++) {
                        const currentSlot = initialslots[slot];
                        const slotDateString = currentSlot.toDateString();

                        if (slotDateString === todayDateString && currentSlot < now) {
                            continue;
                        }

                        const isConflicted = moderators[i].conferences.some(conf => 
                            conf.start === currentSlot.toISOString()
                        );
                        if (isConflicted) continue;

                        let existingSlot = document.getElementById(currentSlot.toISOString());

                        if (existingSlot?.children[0]) {
                            try {
                                let mods = JSON.parse(existingSlot.children[0].dataset.moderators || '[]');
                                if (!mods.includes(moderators[i].email)) {
                                    mods.push(moderators[i].email);
                                    existingSlot.children[0].dataset.moderators = JSON.stringify(mods);
                                }
                            } catch (parseError) {
                                console.error('❌ Error parsing moderators data:', parseError);
                            }
                        } else {
                            let newSlot = document.createElement("div");
                            newSlot.id = currentSlot.toISOString();
                            newSlot.classList.add("flex", "justify-center");
                            
                            const displayTime = new Date(currentSlot);
                            const timeString = displayTime.toLocaleString("en-US", {
                                hour: "numeric",
                                minute: "numeric",
                                hour12: true,
                            });
                            
                            newSlot.innerHTML = `<div data-moderators='["${moderators[i].email}"]'>${timeString}</div><div hidden>${currentSlot.toISOString()}</div>`;
                            newSlot.children[0].classList.add("slotTime");

                            newSlot.addEventListener("click", async () => {
                                try {
                                    const slotDateTime = new Date(newSlot.children[1].innerHTML);
                                    const endDateTime = new Date(slotDateTime.getTime() + duration * 60000);
                                    
                                    document.getElementById("displaySlot").innerHTML =
                                        slotDateTime.toDateString() +
                                        ", " +
                                        ...
                                        slotDateTime.toString().slice(34);
                                    ...
                                    const allSlots = document.getElementsByClassName("slotTime");
                                    for (let tS = 0; tS < allSlots.length; tS++) {
                                        allSlots[tS].classList.remove("active");
                                    }
                                    
                                    newSlot.children[0].classList.add("active");
                                } catch (clickError) {
                                    console.error('❌ Error handling slot click:', clickError);
                                }
                            });

                            const dateContainer = document.getElementById(currentSlot.toDateString());
                            if (dateContainer) {
                                if (dateContainer.children[0]?.innerHTML.includes("No Slots Available")) {
                                    dateContainer.children[0].innerHTML = "";
                                }
                                dateContainer.append(newSlot);
                            }
                        }
                    }
                };

                await processCustomHours();
                displaySlots();
            }

            const monthElement = document.getElementsByClassName("react-datepicker__current-month")[0];
            if (monthElement) {
                setMonthName(monthElement.innerHTML);
            }
        } else {
            setBookedConference(response?.data?.result);
        }
        ...
        let minID, maxID;
        const eventStart = new Date(response?.data?.result?.start);
        const eventEnd = new Date(response?.data?.result?.end);
        const now = new Date();
        
        if (now <= eventStart) {
            setMinDate(response?.data?.result?.start);
            minID = eventStart.toDateString();
            openSlots(eventStart);
        } else {
            setMinDate(now.toISOString());
            minID = now.toDateString();
            openSlots(now);
        }
        
        setMaxDate(response?.data?.result?.end);
        maxID = eventEnd.toDateString();
    ...
    setTimeout(async () => {
      try {
        const response = await axiosPrivate.post(
          `/user/${window.location.href.split("/")[4]}`,
          JSON.stringify({ conference: conference })
        );
          ...
          {bookedConference.link ? (
            <div className="flex w-full h-full justify-center bg-faint-blue items-center certerPageItems pb-20 pt-10">
              <div className="text-center bg-white rounded-2xl light-shadow flex flex-col gap-5 items-center justify-center lg:w-8/12 mx-3 md:mx-8 lg:mx-16 pb-6">
                <div className="">
                  <img src="/conference_Details.png" />
                </div>
                <div className="md:text-lg lg:text-3xl font-bold">
                  🎉 Congratulations!
                </div>
                ...
                <div className="bg-dark-gray rounded-xl pb-6 px-2  mx-2 flex flex-col justify-center items-center gap-2 w-11/12  md:w-10/12">
                  ...
                  <div className="text-xs md:text-base">
                    <span className="font-bold">
                      <CalendarMonthOutlinedIcon /> Date & Time:{" "}
                    </span>
                    &nbsp;{" "}
                    <strong>
                      {new Date(bookedConference.start).toDateString()},{" "}
                      {new Date(bookedConference.start).toLocaleTimeString([], {
                        hour: "2-digit",
                        minute: "2-digit",
                        hour12: true,
                      })}{" "}
                      {new Date(bookedConference.start).toString().slice(34)}
                    </strong>
                  </div>
                  <div className="text-xs md:text-base">
                    <span className="font-bold">
                      <AccessTimeRoundedIcon /> Duration:{" "}
                    </span>
                    &nbsp; 30- 40 Minutes
                  </div>
                  <div className="text-xs md:text-base font-thin text-gray">
                    Copy your interview meeting link{" "}
                  </div>
                  <div className="text-xs md:text-sm lg:text-base bg-light-gray text-gray rounded-lg pl-2 w-full md:w-82 lg:w-96 flex justify-between items-center ">
                    {bookedConference.link} &nbsp; &nbsp;{" "}
                    <button
                      className="h2s-blue-button"
                      onClick={(e) => {
                        e.target.innerHTML = "Copied!";
                        navigator.clipboard.writeText(bookedConference.link);
                      }}
                    >
                      Copy
                    </button>
                  ...
                  <div className="text-xs md:text-base font-thin text-gray">
                    <small className="block">
                      NOTE: Slots once booked are considered final and cannot be
                ...
                <div className="bg-dark-gray rounded-xl pb-5 px-2  mx-2 flex flex-col justify-center items-center gap-2 w-11/12  md:w-10/12">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-2 text-left">
                    <div className="text-xs md:text-base w-12/12">
            ...
            <>
              ...
              <div className="flex-col lg:flex-row w-full mx-5 flex gap-5 justify-center items-start p-2 ">
                  ...
                  <div className=" w-full bg-white rounded-xl light-shadow flex flex-col gap-5 items-center justify-center">
                    <div className="w-full relative">
                      <div className="w-full">
                        <img src="/userEvent/schedule.png" className="w-full" />
                      </div>
                      <div className="nameContainer flex gap-2 items-end relative px-2">
                        <div className="intials bg-google-blue text-2xl">
                          {name?.charAt(0)}
                          {name?.split(" ").length > 1
                            ? name
                                ?.split(" ")
                    ...
                    <div className="w-full bg-dark-gray gap-2 flex flex-col p-3">
                      ...
                      <div className="flex justify-start items-center gap-2">
                        <span className="font-bold">
                          <AccessTimeFilledRoundedIcon fontSize="sm" />{" "}
                          Duration:
                        </span>{" "}
                        {dur} Minutes
                      </div>
                      <div className="flex justify-start items-center gap-2">
                        <span className="font-bold">
                          <CalendarMonthIcon fontSize="sm" /> Schedule:
                        </span>{" "}
                      ...
                      <div className="flex justify-start items-center gap-2">
                        <span className="font-bold">
                          <HelpIcon fontSize="sm" /> Platform Guide:
                        </span>{" "}
                        <Link
                          target="_blank"
                    ...
                    <div className="flex flex-col p-3 gap-2">
                      ...
                      <div id="displaySlot">
                        ...
                        <div className="text-xs">
                          <span className="text-red-500">* </span>Select a time
                          slot in order to book an interview for the event.
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="bg-white rounded-xl flex flex-col w-full md:w-11/12 light-shadow lg:w-4/5 h-full">
                  <div
                    className="bg-google-blue text-google-blue font-bold w-full rounded-t-xl flex justify-center items-center py-4 "
                    id="monthName"
                  >
                    {" "}
                    .
                  ...
                  <div className="bg-white p-2 flex flex-col lg:flex-row gap-5 w-full">
                    ...
                    <div className="w-full lg:w-1/5 flex flex-col gap-5">
                      <div className=" w-full bg-dark-gray rounded-xl">
                        <div className="flex justify-center items-center py-3 border-b-2 border-white border-solid text-lg">
                          <div
                            id="selectedDate"
                      ...
                      <div className="flex w-full justify-center items-center py-2">
                        <button
                          className={
                            conference.start
                              ? "h2s-button gap-1 flex justify-center items-center"
                              : "h2s-button-disabled gap-1 flex justify-center items-center"
                          }
                          id="schdulingButton"
                          onClick={handleClick}
                          disabled={!conference.start || loading}
                        >
                          {loading ? (
                            <CircularProgress
